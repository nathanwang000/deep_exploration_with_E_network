...
def generate_plot_1():
    ...
    for s in seed:
        MSE = (
            MSE
            + lll_learning(
                env, alpha=0.35, epsilon=0.6, num_episodes=num_episodes, seed=s
            )[0]
        )
    MSE = MSE / len(seed)
    font = {"size": 13}
...
def generate_plot_2():
    ...
    for s in seed:
        MSE[0] = (
            MSE[0]
            + lll_learning(
                env, alpha=0.2, epsilon=0.9, num_episodes=num_episodes, seed=s
            )[0]
        )
        MSE[2] = (
            MSE[2]
        ...
        MSE[6] = MSE[6] + lll_counter(
            env, alpha=0.5, epsilon=0.9, num_episodes=num_episodes, seed=s
        )
        MSE[3] = (
            MSE[3]
            + lll_learning(
...
def generate_plot_3_4():
    ...
    num_episodes = 50
    seed = 42
    (MSE, res_E, res_N, res_Q) = lll_learning(
        env,
        alpha=0.5,
        epsilon=0.9,
        num_episodes=num_episodes,
    ...
    for k in range(5):
        ...
        for i in range(num_episodes):
            ...
            info[i] = res_Q[i][s[k] - 8][a]
        plt.plot(
            C, info, marker_style[k], label=r"$\log_{1-\alpha}(E(%d,%d))$" % (s[k], a)
        )
    C = np.zeros(num_episodes)
    for i in range(num_episodes):
        C[i] = res_E[i][9][0]
...
def generate_plot_5():
    ...
    MSE = MSE / np.amax(MSE)
    MSE2 = delayed_Q_learning(
        env, gamma=0.9, m=10, epsilon1=0.01, num_episodes=num_episodes, seed=s
    )
