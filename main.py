...
from lib.action_selection import (
    epsilon_greedy,
    LLL_epsilon_greedy,
    softmax,
    LLL_softmax,
...
def parse_main_args():
    ...
    parser.add_argument(
        "-p", "--plot", action="store_true", help="plot or not", default=False
    )
    parser.add_argument(
        "-a",
        "--selection",
        help="action selection rule",
    ...
    parser.add_argument(
        "-t", "--temperature", help="Temperature for softmax", default=1.0, type=float
    )
...
def op_run(
    ...
    t = Trainer(
        Qnet, env, selection, run_name=run_name, setting=setting, log_path=log_path
    )
    t.run()
...
def dqn_run(
    ...
    selection = {"epsilon": epsilon_greedy(), "softmax": softmax(T=temperature)}[
        selection
    ]
...
def dora_run(
    ...
    selection = {
        ...
        "softmax": LLL_softmax(T=temperature),
    }[selection]
    Qnet = selectNet("dqn", env.name)
...
def run(args):
    env = {
        "cart_pole": CartPoleVision,
        "pacwoman": Pacwoman,
        "mountain_car": MountainCar,
        "bridge": Bridge,
        "mountain_car_long": MountainCarLong,
    }[args.game](render_mode="human" if args.plot else None)
...
if __name__ == "__main__":
    ...
    if args.game == "bridge" and args.mode == "dora":
        joblib.dump(
            EsCounter, os.path.join(args.logpath, "counter_dora_%s.pkl" % args.name)
        )
