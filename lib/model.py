...
class PacwomanDQN(nn.Module):
    def __init__(self):
        ...
        self.regressor = nn.Sequential(nn.Linear(23 * 17 * 32, 9))
...
class CartPoleDQN(nn.Module):
    def __init__(self):
        ...
        self.regressor = nn.Sequential(nn.Linear(448, 2))
...
def selectNet(netname, envname):
    DQN = {
        "cart_pole": CartPoleDQN,
        "pacwoman": PacwomanDQN,
        "mountain_car": MountainCarDQN,
        "mountain_car_long": MountainCarDQN,
        "bridge": BridgeDQN,
    }[envname]
    if netname == "dqn":
        return DQN()
        ...
        for p in Enet.regressor[-1].parameters():
            p.data.fill_(0)
        Enet.regressor = nn.Sequential(
            *(list(Enet.regressor.children()) + [nn.Sigmoid()])
        )
        return Enet
    else:
