...
class Trainer:
    ...
    def optimize_model(self):
        ...
        non_final_mask = BoolTensor(
            tuple(map(lambda s: s is not None, batch.next_action))
        )
            ...
            with torch.no_grad():
                non_final_next_states = Variable(
                    torch.cat(
                        [
                            b
                            for a, b in zip(batch.next_action, batch.next_state)
                            if a is not None
                        ]
                    )
                )
            next_action_batch = Variable(
                torch.cat(
                    [
                        a
                        for a, b in zip(batch.next_action, batch.next_state)
        ...
        expected_state_action_values = torch.unsqueeze(
            (next_state_values * self.gamma) + reward_batch, -1
        )
    ...
    def run(self):
        for i_episode in range(self.num_episodes):
            ...
            for t in count():
                ...
                Qs = (
                    self.model(Variable(state, volatile=True).type(FloatTensor))
                    .data.cpu()
                    .numpy()
                    .ravel()
                )
    ...
    def run(self):
        ...
        for i_episode in range(self.num_episodes):
            ...
            for t in count():
                ...
                with torch.no_grad():
                    Qs = (
                        self.qnet_trainer.model(Variable(state).type(FloatTensor))
                        .data.cpu()
                        .numpy()
                        .ravel()
                    )
                    Es = (
                        self.enet_trainer.model(Variable(state).type(FloatTensor))
                        .data.cpu()
                        .numpy()
                        .ravel()
                    )
                ...
                if done or t >= self.env.spec.max_episode_steps:
                    ...
                    joblib.dump(
                        self.qnet_trainer.rewards,
                        os.path.join(self.log_path, "dora_%s.pkl" % self.run_name),
                    )
